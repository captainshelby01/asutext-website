...
if (lightbox) {
  ...
  // Handle standalone gallery page card elements
  const portfolioElements = document.querySelectorAll('.portfolio-item-element');
  portfolioElements.forEach(item => {
    item.addEventListener('click', (e) => {
      e.stopPropagation();
      const src = item.getAttribute('data-src');
      const type = item.getAttribute('data-type');
      openLightbox(src, type !== 'image', item.querySelector('img')?.alt || '');
    });
  });
